...
from leave.emails import leave_application_email
...
def dashboard_employees_create(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			# email_message = 'You have been added to Swifthub Staff List,username and password'
			# from_email = settings.EMAIL_HOST_USER
			# to_email = [employee_email]
			'''
			Work on it - user@gmail.com & user@swifthub.co.ke -> send Template
			'''
			# send_mail(
			# 	email_subject,
			# 	email_message,
...
def dashboard_employee_info(request,id):
	...
	dataset['title'] = 'profile for' f'{employee.get_full_name}'
...
def dashboard_emergency_create(request):
	...
	dataset['form'] = form
	dataset['title'] = 'Create Emergency Contact'
...
def dashboard_family_create(request):
	if not (request.user.is_authenticated and request.user.is_superuser and request.user.is_staff):
		return redirect('/')
	if request.method == 'POST':
		form = FamilyCreateForm(data = request.POST)
		if form.is_valid():
			instance = form.save(commit = False)
			employee_id = request.POST.get('employee')
			...
			messages.success(request,'Relationship Created Successfully',extra_tags = 'alert alert-success alert-dismissible show')
			return redirect('dashboard:familycreate')
		else:
			messages.error(request,'Failed to create Relationship, kindly ensure you enter data correctly',extra_tags = 'alert alert-warning alert-dismissible show')
			return redirect('dashboard:familycreate')
...
def leave_creation(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			messages.success(request,"Leave Request Sent,kindly wait for your supervisor's response",extra_tags = 'alert alert-success alert-dismissible show')
			leave_application_email(leave_obj=instance)
			return redirect('dashboard:createleave')

		messages.error(request,'Leave Request failed,please double-check your entries',extra_tags = 'alert alert-warning alert-dismissible show')
		return redirect('dashboard:createleave')
...
def approve_leave(request,id):
	...
	messages.error(request,'Leave successfully approved for,' ''f'{leave.user}',extra_tags = 'alert alert-success alert-dismissible show')
...
def reject_leave(request,id):
	...
	leave = get_object_or_404(Leave, id = id)
	leave.reject_leave
	messages.success(request,'Leave is rejected for,' ''f'{ leave.user}',extra_tags = 'alert alert-success alert-dismissible show')
